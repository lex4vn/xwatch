...
class ControllerModulepavhomebuilder extends Controller {
	...
	public function index() {
		...
		$this->load->language('module/pavhomebuilder');
	...
	///	$this->document->addStyle( 'view/javascript/summernote/summernote.css');
		...
		if( isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
			...
			$this->response->redirect($this->url->link('module/pavhomebuilder', 'token=' . $this->session->data['token'], 'SSL'));
		}
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if($action == "save_stay"){
				if( isset($this->request->get['module_id']) ) {
					$this->response->redirect($this->url->link('module/pavhomebuilder', 'module_id='.$this->request->get['module_id'].'&token=' . $this->session->data['token'], 'SSL'));
				}else{
					$this->response->redirect($this->url->link('module/pavhomebuilder', 'token=' . $this->session->data['token'], 'SSL'));
				}
			}else{
		...
		$theme = $storeConfig['config_template'];
	 	...
	 	$dir = DIR_CATALOG.'view/theme/'.$theme.'/template/module/pavhomebuilder';  
	 	$output = array();
	 	if( is_dir($dir) ){
	 		$dir = $dir.'/*.tpl';
	 		$files = glob($dir);
		...
		if (isset($this->request->get['module_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
	 	...
	 	$this->mdata['link'] = $this->url->link('module/pavhomebuilder', 'token=' . $this->session->data['token'] . '', 'SSL');
		...
		$template = 'module/pavhomebuilder/modules.tpl';
		...
		$this->mdata['ifmocmod'] = $this->url->link('module/pavhomebuilder/listmodules', 'token=' . $this->session->data['token'] . '', 'SSL');
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/pavhomebuilder')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
	...
	public function listmodules(){
		...
		$template = 'module/pavhomebuilder/ocmodules.tpl' ;
