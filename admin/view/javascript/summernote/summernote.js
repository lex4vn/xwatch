...
(function (factory) {
  ...
  var settings = {
    ...
    options: {
      ...
      toolbar: [
        ...
        ['fontname', ['fontname']], ['fontsize', ['fontsize']], // Still buggy
        ['color', ['color']],
  ...
  var Editor = function () {
    ...
    this.unlink = function ($editable) {
       ...
       * @param {Object} options
       */
      this.createImageInfo = function ($editable, imageInfo, $target) {
          $target.attr('alt',imageInfo.alt);
          $target.attr('title',imageInfo.title);
          if(imageInfo.responsive){
            $target.addClass('img-responsive');
          }else{
            $target.removeClass('img-responsive');
          }
          
          var nextElement = $target.next();
          if (nextElement.prop('tagName') == 'FIGCAPTION') {
            nextElement.html(imageInfo.caption);
          }else{
            $target.after('<figcaption>'+imageInfo.caption+'</figcaption>');
          }
     ...
     * @return {Object}
     */
    this.getImageInfo = function ($editable, $target) {
      ...
      // var rng = range.collapse(dom.isImg());

      // var image = dom.ancestor(rng.sc, dom.isImg());
      ...
      return {
          ...
          src: $target.attr('src'),
          alt: $target.attr('alt'),
          caption: $target.next().html(),
          responsive: $target.hasClass('img-responsive')
      };
    };
    /**
     * returns link info
  ...
  var Dialog = function () {
    ...
    this.showVideoDialog = function ($editable, $dialog, videoInfo) {
       ...
       * @return {Promise}
       */
      this.showImageEditDialog = function ($editable, $dialog, imageInfo) {
          return $.Deferred(function (deferred) {
              var $imageEditDialog = $dialog.find('.note-image-edit-dialog');

              var $imageAlt = $imageEditDialog.find('.note-image-alt'),
                  $imageTitle = $imageEditDialog.find('.note-image-title'),
                  $editBtn = $imageEditDialog.find('.note-image-edit-btn'),
                  $caption = $imageEditDialog.find('.note-image-caption'),
                  $responsive = $imageEditDialog.find('input[type=checkbox]');
                  
              $imageEditDialog.one('shown.bs.modal', function () {
                  $caption.val(imageInfo.caption);
                  $imageAlt.val(imageInfo.alt);
                  $imageTitle.val(imageInfo.title);
                  $responsive.prop('checked', imageInfo.responsive);
                  $editBtn.one('click', function (event) {
                      event.preventDefault();
                      deferred.resolve({
                          ...
                          caption: $caption.val(),
                          responsive: $responsive.is(':checked')
                      });
  ...
  var EventHandler = function () {
    ...
    var commands = {
      ...
      showLinkDialog: function (layoutInfo) {
         ...
         * @param {Object} layoutInfo
         */
        showImageEditDialog: function (layoutInfo) {
            var $handle = layoutInfo.handle();
            var target = $handle.find('.note-control-selection').data('target'),
                $target = $(target);
            var $editor = layoutInfo.editor(),
                $dialog = layoutInfo.dialog(),
                $editable = layoutInfo.editable(),
                imageInfo = editor.getImageInfo($editable,$target);

            editor.saveRange($editable);
            dialog.showImageEditDialog($editable, $dialog, imageInfo).then(function (imageInfo) {
                editor.restoreRange($editable);
                editor.createImageInfo($editable, imageInfo, $target);
                // hide popover after creating image
                popover.hide(layoutInfo.popover());
            }).fail(function () {
                editor.restoreRange($editable);
            });
    ...
    var hToolbarAndPopoverClick = function (event) {
      ...
      if ($btn.length) {
        ...
        var $target;
        if ($.inArray(eventName, ['resize', 'floatMe', 'removeMedia', 'showImageEditDialog']) !== -1) {
  ...
  var Renderer = function () {
    ...
    var tplPopovers = function (lang, options) {
      ...
      var imageEditButton = tplIconButton('fa fa-edit icon-edit', {
          title: lang.image.edit,
          event: 'showImageEditDialog'
      });
        ...
        var content = '<div class="btn-group">' +imageEditButton+ fullButton + halfButton + quarterButton + '</div>' +
    ...
    var tplDialogs = function (lang, options) {
      ...
      var tplLinkDialog = function () {
        ...
        var tplImageEditDialog = function () {
            var body = '<div class="form-group">' +
                '<label>Thẻ alt - Mô tả của ảnh chứa từ khóa cần SEO</label>' +
                '<input class="note-image-alt form-control span12" type="text" />' +
                '</div>' +
                '<div class="form-group">' +
                '<label>Thẻ title - Tiêu đề của ảnh chứa từ khóa cần SEO</label>' +
                '<input class="note-image-title form-control span12" type="text" />' +
                '</div>' +
                '<div class="form-group">' +
                '<label>Caption - Nội dung mô tả ảnh chứa từ khóa cần SEO</label>' +
                '<input class="note-image-caption form-control span12" type="text" />' +
                '</div>' +
                '<div class="checkbox">' +
                '<label>' + '<input type="checkbox" checked> Responsive</label>' +
                '</div>';
            var footer = '<button href="#" class="btn btn-primary note-image-edit-btn">Cập nhật</button>';
            return tplDialog('note-image-edit-dialog', 'Cập nhật thông tin ảnh', body, footer);
        };
