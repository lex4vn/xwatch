...
class ControllerModulePavdeals extends Controller {
	...
	public function index($setting) { 
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/pavdeals/carousel_deals.tpl')) {
			$template = $this->config->get('config_template') . '/template/module/pavdeals/carousel_deals.tpl';
		} else {
			$template = 'default/template/module/pavdeals/carousel_deals.tpl';
		}
	...
	public function getItemDeal($product = null, $setting = array()){
		...
		if ($this->config->get('config_tax')) {
			$tax = $this->currency->format((float)$deal['special'] ? $deal['special'] : $deal['price']);
		} else {
			$tax = false;
		}
