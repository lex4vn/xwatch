...
class ModelSettingSetting extends Model
{
    ...
    public function editByKey($key, $value)
    {
        if (!$this->getValueByKey($key)) {
            $this->db->query("INSERT INTO " . DB_PREFIX . "setting SET store_id = '0', `code` = 'config', `key` = '" . $this->db->escape($key) . "', `value` = '" . $this->db->escape($value) . "'");
        } else {
